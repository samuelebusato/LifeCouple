...
| Componente | STRIDE | Minaccia concreta | Conseguenza | Mitigazione |
|---|---|---|---|---|
| Diritto in tabella | **E** | Il client scrive o modifica la colonna del diritto e si concede «Insieme» | Funzioni a pagamento gratis per chiunque sappia usare l'API col proprio token | **Nessuna policy di scrittura** su quella colonna: la tocca solo il `service_role` dalla Edge Function del webhook. La RLS del client la espone in **sola lettura** · **da fare** |
| Stato dell'SDK | **S** | L'app modificata dichiara di avere il diritto e sblocca le funzioni da sé | Le stesse funzioni gratis, senza nemmeno toccare il backend | 🔑 **Il gate sta nel database, non nell'app**: le funzioni a pagamento devono dipendere da policy o da query che leggono **la nostra colonna**, non da `customerInfo`. Ciò che l'SDK dice serve solo a scegliere cosa disegnare · **da fare** |
